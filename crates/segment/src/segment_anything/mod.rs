// pub use candle_transformers::models::with_tracing::Linear;
...
pub fn linear(vb: VarBuilder, in_dim: usize, out_dim: usize, bias: bool) -> Result<Linear> {
    if bias {
        candle_transformers::models::with_tracing::linear(in_dim, out_dim, vb)
    } else {
        candle_transformers::models::with_tracing::linear_no_bias(in_dim, out_dim, vb)
    }
}
