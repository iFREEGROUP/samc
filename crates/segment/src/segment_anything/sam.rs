...
impl Sam {
    ...
    pub fn preprocess(&self, img: &Tensor) -> Result<Tensor> {
        ...
        if h > IMAGE_SIZE || w > IMAGE_SIZE {
            candle_core::bail!("image is too large ({w}, {h}), maximum size {IMAGE_SIZE}")
        }
    ...
    fn process_crop(
        ...
        cb: CropBox,
        point_grids: &[(f64, f64)],
    ) -> Result<Vec<object_detection::Bbox<Tensor>>> {
        ...
        for points in points.chunks(64) {
            ...
            for (i, iou) in iou_predictions.iter().enumerate() {
                ...
                if let Some(((x0, x1), (y0, y1))) = min_max_x.zip(min_max_y) {
                    let bbox = object_detection::Bbox {
                        xmin: x0 as f32,
                        ymin: y0 as f32,
                        xmax: x1 as f32,
                        ymax: y1 as f32,
        ...
        object_detection::non_maximum_suppression(&mut bboxes, CROP_NMS_THRESH);
